...
7. Sales by State (Geographic Map)
...
1.	Non-Veg dominates but Veg is not far behind. At 35%, Veg orders are a substantial segment
  	worth targeting with dedicated promotions.

2.	Ratings are uniformly high. An average of 4.34 across all three quarters — with zero variation
  	— suggests either very consistent service quality, or that the rating distribution warrants
  	further investigation (potential rating inflation).

3.	Weekends are the revenue peak. Saturday–Sunday consistently outperform weekdays. Promotions
  	and offers timed around weekends would likely see higher conversion.

4.	Bengaluru is in a league of its own. At ₹5.46M it accounts for roughly 10% of all platform sales.
  	Understanding what drives Bengaluru's volume — restaurant density, order frequency, or higher
  	AOV — would be valuable.

5.	Average Order Value is ₹268. This sits in the mid-range for food delivery. There's likely an
  	opportunity to push AOV higher through bundling or upselling, particularly on weekday orders
  	where basket sizes tend to be smaller.
